...
void ConfigLoader::validate(const SimParams &params) {
  ...
  if (params.model.D1 < 0.0f || params.model.D2 < 0.0f ||
      params.model.D3 < 0.0f) {
  ...
  if (!params.is_stable()) {
    ...
    float dt_max = (params.DIMENSION == 1)
                       ? (params.dx * params.dx) / (2.0f * Dmax)
                       : (params.dx * params.dx) / (4.0f * Dmax);
    ...
    std::cerr << "  Consider reducing dt or using implicit scheme."
              << std::endl;
  }

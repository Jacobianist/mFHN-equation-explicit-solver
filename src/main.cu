...
void generate_1d(std::vector<float> &u, std::vector<float> &v,
                 std::vector<float> &w, const SimParams &params) {
  // u.resize(params.N, 0.0f);
  // v.resize(params.N, 0.0f);
  // w.resize(params.N, 0.0f);
...
void generate_2d(std::vector<float> &u, std::vector<float> &v,
                 std::vector<float> &w, const SimParams &params) {
  // size_t size = params.N * params.N;
  // u.resize(size, 0.0f);
  // v.resize(size, 0.0f);
  // w.resize(size, 0.0f);
...
int main() {
  ...
  std::cout << "Config loaded. N=" << params.N << ", dt=" << params.dt
            << ", dx=" << params.dx << ", alpha=" << params.model.alpha
            << ", ksi=" << params.dt / (4 * params.dx * params.dx) << std::endl;

  std::vector<float> h_u(params.size_total()), h_v(params.size_total()),
      h_w(params.size_total());
  ...
  { //! REWORK
    ...
    hid_t file_id =
        H5Fopen(params.init_file.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    ...
    herr_t status = H5Dread(dataset_id, H5T_NATIVE_FLOAT, mem_space_id,
                            space_id, H5P_DEFAULT, h_u.data());
  ...
  cudaMemcpy(d_u, h_u.data(), params.size_total() * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_v, h_v.data(), params.size_total() * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_w, h_w.data(), params.size_total() * sizeof(float),
             cudaMemcpyHostToDevice);
  ...
  // model.D3); if (ITERATION_STEP % saveInterval == 0 || ITERATION_STEP ==
  // params.steps) {
  //     exporter.save_step(snapshot_idx++, d_u, d_v, d_w);
  // }
  // }

  std::cout << "TWO  " << params.DIMENSION << std::endl;
  ...
  if (params.DIMENSION == 1) {
    ...
    for (int ITERATION_STEP = 1; ITERATION_STEP <= params.steps;
         ++ITERATION_STEP) {
      gpu_rk4_1d<<<blocks, 256>>>(d_u_next, d_v_next, d_w_next, d_u, d_v, d_w,
                                  params, params.N, params.dt);
      if (params.model.D1 > 0) {
        diffusion_1d_explicit<<<blocks, 256>>>(d_u, d_u_next, params.N,
                                               params.r_u());
      } else {
        std::swap(d_u, d_u_next);
      }
      if (params.model.D1 > 0) {
        diffusion_1d_explicit<<<blocks, 256>>>(d_v, d_v_next, params.N,
                                               params.r_v());
      } else {
        std::swap(d_v, d_v_next);
      }
      if (params.model.D1 > 0) {
        diffusion_1d_explicit<<<blocks, 256>>>(d_w, d_w_next, params.N,
                                               params.r_w());
      } else {
    ...
    for (int ITERATION_STEP = 1; ITERATION_STEP <= params.steps;
         ++ITERATION_STEP) {
      gpu_rk4_2d<<<blocks, threads>>>(d_u_next, d_v_next, d_w_next, d_u, d_v,
                                      d_w, params, params.N, params.dt);

      diffusion_2d_explicit<<<blocks, threads>>>(d_u, d_u_next, params.N,
                                                 params.r_u());
      diffusion_2d_explicit<<<blocks, threads>>>(d_v, d_v_next, params.N,
                                                 params.r_v());
      diffusion_2d_explicit<<<blocks, threads>>>(d_w, d_w_next, params.N,
                                                 params.r_w());
  ...
  cudaMemcpy(h_u.data(), d_u, params.size_total() * sizeof(float),
